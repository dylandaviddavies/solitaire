...
- **(bug) A click on the empty stock when the waste is also empty still counts as a move.** `GameEngine.draw()` runs a `RecycleMove` unconditionally when the stock is empty, so with nothing to recycle it pushes a no-op onto the undo history, bumps the move counter, and the UI plays the shuffle sound and flourish. Guard `draw()` (or `handleDraw`) with `waste.isEmpty`.
- **(bug) Double-click-to-foundation is vestigial and can act on the wrong card.** The first click of a double-click already fires `onClickMove` → `autoMove`, which prefers foundations — so by the time `onDoubleClick` fires, the card has usually moved and the second click/dblclick lands on the card revealed beneath it (which can then also move). Either remove the `onActivate` path and its README line, or debounce the single-click move behind the double-click window.
