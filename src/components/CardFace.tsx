import { SUIT_SYMBOL, type Card } from '../domain/Card'
  ...
  return (
    <div className="relative flex h-full w-full flex-col justify-between rounded-[14px] bg-[#fffaf0] p-2 select-none">
      <div className={`flex flex-col items-start self-start leading-none ${colorClass}`}>
        ...
        <span className="text-[1.05rem]">{SUIT_SYMBOL[card.suit]}</span>
      </div>
      ...
      <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
        {isFace ? (
          ...
          <span className={`text-[2.6rem] opacity-90 ${colorClass}`}>{SUIT_SYMBOL[card.suit]}</span>
        )}
      </div>
